    ...
    def initialize_processing(self):
        ...
        try:
            ...
            for step in range(1, MAX_EXPLORATION_STEPS + 1):
                ...
                if choice < 0.5:
                    # Move forward at 0.3 m/s for 0.5 seconds = 0.15m total
                    action_type = "velocity_control"
                    parameters = {
                        "linear_velocity": [0.3, 0.0, 0.0],  # 0.3 m/s forward (x-axis)
                        "angular_velocity": [0.0, 0.0, 0.0],  # No rotation
                        "duration": 0.5,  # Execute for 0.5 seconds
                    }
                    ...
                    parameters = {
                        ...
                        "angular_velocity": [0.0, 0.0, 0.7],  # 0.7 rad/s left (z-axis)
                        "duration": 0.5,  # Execute for 0.5 seconds
                    }
                    # 🎯 ADD METADATA (SAME AS ACTION EXECUTOR!)
                    velocity_metadata = {
                    ...
                    parameters = {
                        "linear_velocity": [0.0, 0.0, 0.0],  # No forward movement
                        "angular_velocity": [0.0, 0.0, -0.7],  # -0.7 rad/s right (z-axis)
                        "duration": 0.5,  # Execute for 0.5 seconds
                    }
                    # 🎯 ADD METADATA (SAME AS ACTION EXECUTOR!)
                    velocity_metadata = {
                ...
                # ⏰ Sleep between exploration steps (already accounted for in duration)
                time.sleep(0.1)  # Reduced from 0.5s since velocity command has its own duration
            ...
            # # ✅ REAL: Set up mission tracking
            # mission_goals = ["explore_area", "build_map", "navigate_safely"]
            # if hasattr(self.task_store, "set_mission_goals"):
            #     self.task_store.set_mission_goals(mission_goals)
