...
class ReasoningPipeline:
    ...
    def on_fused_predictions_ready(self, event_type: str, data: Dict):
        ...
        if event_type == "fused_predictions_ready":
            ...
            if self.trigger_count == 1:
                print(f"📊 ReasoningPipeline: Trigger count: {self.trigger_count} - running reasoning cycle")
                # Auto-trigger reasoning cycle
                reasoning_cycle_id = data.get(
                    "reasoning_cycle_id", 0
                )
                self.run_reasoning_cycle()
            else:
                print(f"📊 ReasoningPipeline: Trigger count: {self.trigger_count} - already processed, skipping")
        ...
        try:
            ...
            if self.tree_of_thoughts:
                ...
                if tree_result:
                    ...
                    print(f"  🔍 DEBUG: Tree of Thoughts output: {tree_result}")  # << ADD THIS
            ...
            if self.spatial_reasoning:
                ...
                if spatial_result:
                    ...
                    print(f"  🔍 DEBUG: Spatial Reasoning output: {spatial_result}")  # << ADD THIS


            print(f"📊 ReasoningPipeline: Components completed - {list(all_component_data.keys())}")
    ...
    def _fuse_all_component_reasoning(self, all_component_data: Dict) -> Dict:
        ...
        if tree_data and spatial_data:
            ...
            search_pattern = detailed_plan.get("search_pattern", "proximity_first_pattern")
            
            # ✅ Spatial Reasoning data extraction
            spatial_priority_list = spatial_data.get("search_strategy", {}).get("priority_list", [])
            top_priority = spatial_data.get("search_strategy", {}).get("top_priority", {})
            ...
            if spatial_priority_list:
                for priority_item in spatial_priority_list[:2]:
                    ...
                    obj_coordinates = priority_item.get("coordinates")  # 🎯 GET COORDINATES!
                    ...
                    if obj_name:
                        planned_actions.append({
                            ...
                            "parameters": {
                                ...
                                "target_coordinates": obj_coordinates,  # 🎯 ADD COORDINATES!
            ...
            if target_objects:
                for obj in target_objects:  # Limit to 3 for stability
                    ...
                    target_coordinates = spatial_item.get("coordinates") if spatial_item else None
            ...
            action_with_coords = next(
                (a for a in planned_actions if a.get("parameters", {}).get("target_coordinates") is not None),
                None
            )

            selected_action = action_with_coords if action_with_coords else planned_actions[0]


            
            # ✅ BACKWARD COMPATIBLE confidence calculation
            tree_confidence = tree_data.get("strategy_decision", {}).get("confidence", 0.5)
            spatial_safe_paths = spatial_data.get("summary_metrics", {}).get("safe_navigation_paths", 0)
            ...
            fused_result = {
                ...
                "components_contributing": ["tree_of_thoughts", "spatial_reasoning"],  # ✅ Backward compatible
                "execution_readiness": "ready" if final_confidence > 0.6 else "needs_confirmation",  # ✅ Backward compatible
                # ✅ ADD RICH DATA IN EXISTING FIELD (backward compatible)
                "plan_metadata": {  
                    "target_objects": target_objects,
                    "search_pattern": search_pattern,
                    "spatial_priorities": [item.get("name") for item in spatial_priority_list[:3]],
                    "top_priority": top_priority.get("name") if isinstance(top_priority, dict) else None
                },
                        "has_coordinates": any("coordinates" in item for item in spatial_priority_list),  # 🎯 NEW
                    "target_objects": target_objects,
        ...
        # 2️⃣ CASE: ONLY TOT AVAILABLE (backward compatible)
            ...
            fused_result = {
                ...
                "execution_readiness": "ready" if confidence > 0.6 else "needs_confirmation",
                    "target_objects": target_objects
        ...
        # 3️⃣ CASE: ONLY SPATIAL REASONING (backward compatible)
            ...
            target_objects = [item.get("name") for item in spatial_priority_list if "name" in item]
            ...
            fused_result = {
                ...
                "execution_readiness": "cautious" if spatial_confidence > 0.4 else "needs_confirmation",
                "target_objects": target_objects
            }
        ...
        # 4️⃣ EMERGENCY FALLBACK with 50+ actions
            ...
            print("❌ FUSING: No component data available - creating 50+ exploration actions")
